...
public class GridMapCreator : EditorWindow
{
    ...
    private void SetUpMap(GameObject _theMap)
    {
        ...
        try 
        { 
            _theMap = GameObject.FindGameObjectWithTag("Map");
            RestoreTheMap(_theMap);
    ...
    private void SetUpStyles()//Sets styles for the different buttons based on what you give it in the inspector.
    {
        try
        {
    ...
    private void SetUpNodesAndParts()
    {
        ...
        for (int i = 0; i < 20; i++)
        {
            ...
            for (int j = 0; j < 10; j++)
            {
                ...
                nodes[i].Add(new Node(nodePos, 30, 30, empty));//Sets up all the nodes in the window and makes their GUIstyle "empty" which is the empty backgrounf.
                parts[i].Add(null);
            }
        }
    }
    ...
    private void ProcessNodes(Event e)//Updates what is shown on the grid when you try to add or erase something.
    {
    ...
    private void PaintNodes(int row, int column)//This is the function that actually draws / removes the objects from the green nodes.
    {
        //Debug.Log("2");
        if (isErasing)
        {
            if (parts[row][column] != null)//If there is an object on that particular node.
            {
                nodes[row][column].SetStyle(empty);
        ...
        else
        {
            if (parts[row][column] == null)//If there's nothing on that node the user clicked on, then do something.
            {
                ...
                GameObject g = Instantiate(Resources.Load("MapParts/" + currentStyle.normal.background.name)) as GameObject;//Spawns the currentStyle object.
                g.name = currentStyle.normal.background.name;
                g.transform.position = new Vector3(column * 10, 0, row * 10) + Vector3.forward * 5 + Vector3.right * 5;//The object is moved to the position of the node the user clicked on.
                GameObject theMap = GameObject.FindGameObjectWithTag("Map");
